                                                ...
                                                <FormItem className={styles.attributeBox}>
                                                    ...
                                                    <FormControl>
                                                        <Input
                                                            ...
                                                            min="0"
                                                            max={field.name === 'luck' ? "100" : field.name === 'sanity' ? "99" : undefined}
                                                            onKeyDown={(e) => {
                                                                if (e.key === '-' || e.key === 'e') {
                                                                    e.preventDefault();
                                                                }
                                ...
                                {fieldConfigs.personalInfo.fields
                                    ...
                                    .map((field) => (
                                        <FormField
                                            key={field.name}
                                            control={form.control}
                                            name={`personalInfo.${field.name}` as keyof SheetFormValues}
                                            render={({ field: formField }) => (
                                ...
                                <div className={styles.attributesGrid}>
                                    {fieldConfigs.characteristics.fields.map((field) => (
                                        <FormField
                                            key={field.name}
                                            control={form.control}
                                            name={`characteristics.${field.name}` as keyof SheetFormValues}
                                            render={({ field: formField }) => (
                                                <FormItem className={styles.attributeBox}>
                                                    <div className={styles.attributeLabel}>
                                                    ...
                                                    <FormControl>
                                                        <Input
                                                            ...
                                                            onChange={e => {
                                                                ...
                                                                value = parseInt(value.toString().slice(0, 2)) || 0;
                                                                formField.onChange(value);
                                                            }}
                                                            className={styles.mainValue}
                                                            min="0"
                                    ...
                                    <div key={category} className={styles.skillGroup}>
                                        ...
                                        {config.fields.map((field) => (
                                            <FormField
                                                key={field.name}
                                                control={form.control}
                                                name={`skills.${field.name}` as `skills.${typeof field.name}` & keyof SheetFormValues}
                                                render={({ field: formField }) => (
                                                    <SkillBox
                                                        label={field.label}
